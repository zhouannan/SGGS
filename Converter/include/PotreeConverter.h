...
inline vector<Attribute> computeOutputAttributes(PlyHeader &header, Options options)
{
	std::unordered_map<std::string, AttributeType> typeMap = {
		{"getFloat64", AttributeType::DOUBLE},
		{"getInt32", AttributeType::INT32},
		{"getUint32", AttributeType::UINT32},
		{"getFloat32", AttributeType::FLOAT},
		{"getInt16", AttributeType::INT16},
		{"getUint16", AttributeType::UINT16},
		{"getUint8", AttributeType::UINT8},
		{"getInt8", AttributeType::INT8}};
	vector<Attribute> lists;
	Attribute xyz("position", 3 * header.bytes["x"], 3, header.bytes["x"], typeMap[header.types["x"]]);
	lists.push_back(xyz);
	if (header.types.find("nx") != header.types.end())
	{
		Attribute normal("normal", 3 * header.bytes["nx"], 3, header.bytes["nx"], typeMap[header.types["nx"]]);
		lists.push_back(normal);
	}
	Attribute rgb("rgb", 3 * header.bytes["f_dc_0"], 3, header.bytes["f_dc_0"], AttributeType::FLOAT);
	lists.push_back(rgb);
	if (header.types.find("f_rest_0") != header.types.end() && options.SH >= 1)
	{
		Attribute SH1("SH1", 9 * header.bytes["f_rest_0"], 9, header.bytes["f_rest_0"], typeMap[header.types["f_rest_0"]]);
		lists.push_back(SH1);
	}
	if (header.types.find("f_rest_9") != header.types.end() && options.SH >= 2)
	{
		Attribute SH2("SH2", 15 * header.bytes["f_rest_9"], 15, header.bytes["f_rest_9"], typeMap[header.types["f_rest_9"]]);
		lists.push_back(SH2);
	}
	if (header.types.find("f_rest_24") != header.types.end() && options.SH >= 3)
	{
		Attribute SH3("SH3", 21 * header.bytes["f_rest_24"], 21, header.bytes["f_rest_24"], typeMap[header.types["f_rest_24"]]);
		lists.push_back(SH3);
	}
	Attribute opacity("opacity", 4, 1, header.bytes["opacity"], typeMap[header.types["opacity"]]);
	Attribute scale("scale", 3 * header.bytes["scale_0"], 3, header.bytes["scale_0"], typeMap[header.types["scale_0"]]);
	Attribute rotation("rotation", 4 * header.bytes["rot_0"], 4, header.bytes["rot_0"], typeMap[header.types["rot_0"]]);
	lists.push_back(opacity);
	lists.push_back(scale);
	lists.push_back(rotation);
	if (header.types.find("cluster_label") != header.types.end() && options.hasCluster)
	{
		Attribute clusterLabel("clusterLabel", 1 * header.bytes["cluster_label"], 1, header.bytes["cluster_label"], typeMap[header.types["cluster_label"]]);
		lists.push_back(clusterLabel);
	}

	return lists;
}

inline Attributes computeOutputAttributes(vector<Source> &sources, vector<string> requestedAttributes, Options options)
{
	...
	// compute scale and offset from all sources
	{
		...
		for_each(parallel, sources.begin(), sources.end(), [&mtx, &sources, &scaleMin, &min, &max, requestedAttributes, &fullAttributeList, &acceptedAttributeNames, options](Source &source)
				 {

			string extension = path(source.path).extension().string();
			vector<Attribute> attributes;
			if(extension == ".ply"){
				auto header = loadPlyHeader(source.path);
				attributes = computeOutputAttributes(header,options);
				scaleMin.x = 1;
				scaleMin.y = 1;
				scaleMin.z = 1;
				source.min = header.min;
				source.max = header.max;
				...
				attributes = computeOutputAttributes(header);
