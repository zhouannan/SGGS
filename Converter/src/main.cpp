...
Options parseArguments(int argc, char **argv)
{
	...
	args.addArgument("fileType", "The type of file is .ply or .las");
	args.addArgument("SH", "The Order of SH");
	args.addArgument("has-cluster", "whether gs has cluster label");
	args.addArgument("cluster-statistics", "statistics file of cluster point");
	...
	string method = args.get("method").as<string>("random");
	string chunkMethod = args.get("chunkMethod").as<string>("LASZIP");
	string fileType = args.get("fileType").as<string>(".las");
	string clusterStatistics = args.get("cluster-statistics").as<string>();

	int SH = args.get("SH").as<int>(0);
	...
	bool hasCluster = args.has("has-cluster");
	...
	options.clusterStatistics = clusterStatistics;
	// options.flags = flags;
...
Curated curateSources(vector<string> paths)
{
	...
	for (auto path : paths)
	{
		...
		{
			if (iEndsWith(path, "las") || iEndsWith(path, "laz") || iEndsWith(path, "ply"))
			{
	...
	for_each(parallel, paths.begin(), paths.end(), [&mtx, &sources](string path)
		...
		if(extension == ".ply"){
			auto header = loadPlyHeader(path);
...
void chunking(Options &options, vector<Source> &sources, string targetDir, Stats &stats, State &state, Attributes outputAttributes, Monitor *monitor, std::unordered_map<int, chunker_countsort_laszip::ClusterInfo> clusterInfoMap)
{
	...
	if (options.chunkMethod == "LASZIP")
	{
		...
		chunker_countsort_laszip::doChunking(sources, targetDir, stats.min, stats.max, state, outputAttributes, monitor, options, clusterInfoMap);
	}
...
void indexing(Options &options, string targetDir, State &state, std::unordered_map<int, chunker_countsort_laszip::ClusterInfo> clusterInfoMap)
{
	if (options.noIndexing)
	{
		return;
	}
	if (options.method == "cluster" && options.hasCluster)
	{
		SamplerCluster sampler;
		indexer::doIndexing(targetDir, state, options, sampler, clusterInfoMap);
	}
...
int main(int argc, char **argv)
{
	...
	auto outputAttributes = computeOutputAttributes(sources, options.attributes, options);
	...
	{ // this is the real important stuff
		std::unordered_map<int, chunker_countsort_laszip::ClusterInfo> clusterInfoMap;
		if (options.clusterStatistics.size() > 0)
		{
			string statisticsText = readTextFile(options.clusterStatistics);
			json js = json::parse(statisticsText);
			for (auto cluster : js.items())
			{
				chunker_countsort_laszip::ClusterInfo clusterInfo;
				string key = cluster.key();
				auto value = cluster.value();
				clusterInfo.min = {
				...
				clusterInfo.count = value["count"];
				clusterInfo.dis = value["dis"];
				clusterInfoMap[std::stoi(key)] = clusterInfo;
			}
		}

		chunking(options, sources, targetDir, stats, state, outputAttributes, monitor.get(),clusterInfoMap);

		indexing(options, targetDir, state, clusterInfoMap);
	}
