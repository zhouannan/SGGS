...
struct SamplerRandom : public Sampler
{
				...
				Options options)
	{
		...
		traversePost(node, [bytesPerPoint, baseSpacing, scale, offset, &onNodeCompleted, &onNodeDiscarded, attributes, options](Node *node)
					 {
			node->sampled = true;
			...
			for (int childIndex = 0; childIndex < 8; childIndex++) {
				...
				for (int i = 0; i < child->numPoints; i++) {
					...
					if(options.fileType == ".ply"){
						float_t* xyz = reinterpret_cast<float_t*>(child->points->data_u8 + pointOffset);

						double x = (xyz[0] * scale.x) + offset.x;
						double y = (xyz[1] * scale.y) + offset.y;
						double z = (xyz[2] * scale.z) + offset.z;
						cellIndex = toCellIndex({ x, y, z });
					}else{
