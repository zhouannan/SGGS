...
namespace indexer
{
	...
	struct ChunkNode
	{
		...
		ChunkNode(string id, int numPoints)
		{
			this->id = id;
			this->numPoints = numPoints;
		}

		ChunkNode(string id, int numPoints, std::unordered_map<int, int> clusterMap)
		{
			this->id = id;
			this->numPoints = numPoints;
	...
	vector<double> getLevelSampleRate(std::unordered_map<int, chunker_countsort_laszip::ClusterInfo> clusterInfoMap,vector<unordered_map<int, double>>sampleData){
		vector<double>result(sampleData.size());
		for(int i=1;i<sampleData.size();i++){
			double total = 0;
			double weightTotal = 0;
			unordered_map<int, double>levelClusterRate = sampleData[i];
			for(auto pair:levelClusterRate){
				total += clusterInfoMap[pair.first].count;
				weightTotal += pair.second * clusterInfoMap[pair.first].count;
			}
			double levelSampleRate = (0.6 * total) / weightTotal;
			result[i] = levelSampleRate;
		}
		return result;
	}

	vector<unordered_map<int, double>> getSampleData(string pathIn)
	{
		string filePath = pathIn + "/sampleData.json";
		ifstream file(filePath);
		if (!file)
		{
			cerr << "Error: Sample File not found: " << filePath << endl;
			return vector<unordered_map<int, double>>(); // 返回一个空的结果
		}
		string sampleDataText = readTextFile(filePath);
		json js = json::parse(sampleDataText);
		vector<unordered_map<int, double>> result(js.size() + 1);
		for (auto &el : js.items())
		{
			int level = std::stoi(el.key());
			json levelInfo = el.value();
			for (auto &sampleInfo : levelInfo)
			{
				for (auto &pair : sampleInfo.items()) {
            int category = std::stoi(pair.key());
            double rate = pair.value().get<double>(); // 直接获取值，应该是数字
            result[level][category] = rate; // 存储到 result 中
        }
			}
		}
		return result;
	...
	string Indexer::createMetadata(Options options, State &state, Hierarchy hierarchy)
	{
		...
		auto getAttributesJsonString = [&attributes, t, s, toJson, vecToJson, vecI64ToJson, options]()
		{
			...
			for (int i = 0; i < attributes.list.size(); i++)
			{
				...
				if (attribute.numElements == 1 && options.fileType != ".ply")
				{
				...
				else if (attribute.numElements == 2 && options.fileType != ".ply")
				{
				...
				else if (attribute.numElements == 3 && options.fileType != ".ply")
				{
		...
		if (options.fileType == ".ply")
		{
			ss << t(1) << s("fileType") << ": " << s(options.fileType) << "," << endl;
		}
	...
	void buildHierarchy(Indexer *indexer, Node *node, shared_ptr<Buffer> points, int64_t numPoints, int64_t depth, Options options)
	{
		...
		int64_t sizePoints = points->size;
		...
		auto gridIndexOf = [&points, bpp, scale, offset, min, size, counterGridSize, options](int64_t pointIndex)
		{
			int64_t pointOffset = pointIndex * bpp;
			if (options.fileType == ".ply")
			{
				float_t *xyz = reinterpret_cast<float_t *>(points->data_u8 + pointOffset);
		...
		for (int64_t nodeIndex = 0; nodeIndex < needRefinement.size(); nodeIndex++)
		{
			...
			buildHierarchy(indexer, subject, buffer, nextNumPoins, depth + 1, options);
		}
	}
	...
	void doIndexing(string targetDir, State &state, Options &options, Sampler &sampler,std::unordered_map<int, chunker_countsort_laszip::ClusterInfo> clusterInfoMap)
	{
		...
		auto sampleData = getSampleData(targetDir);
		options.sampleRate = sampleData;

		vector<double> levelSampleRate;

		if(options.clusterStatistics.size() > 0){
			levelSampleRate = getLevelSampleRate(clusterInfoMap,sampleData);
			options.levelSampleRate = levelSampleRate;
		}
		...
		buildHierarchy(&indexer, chunkRoot.get(), pointBuffer, numPoints, 0, options);

		sampler.sample(chunkRoot.get(), attributes, indexer.spacing, onNodeCompleted, onNodeDiscarded,options);
		...
		{ // process chunk roots in batches
			...
			for (auto &task : tasks)
			{
				...
				sampler.sample(task.node, attributes, indexer.spacing, onNodeCompleted, onNodeDiscarded, options);
		...
		{
			sampler.sample(indexer.root.get(), attributes, indexer.spacing, onNodeCompleted, onNodeDiscarded, options);
		}
